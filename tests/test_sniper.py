...
class _FakeSupabaseClient:
    ...
    async def insert_row(self, schema: str, table: str, row: dict) -> dict:
        self.rows.append((schema, table, row))
...
def wired(monkeypatch):
    ...
    with TestClient(app) as client:
        yield client, fake_supa, fake_tg
...
def test_valid_alert_persists_and_broadcasts(wired):
    client, fake_supa, fake_tg = wired
    ...
    # Persisted into the configured schema/table.
    assert len(fake_supa.rows) == 1
    schema, table, row = fake_supa.rows[0]
    assert schema == "aurum-customers"
